...
import com.qidizi.netSniffer.R;
...
public class KcaVpnService extends VpnService {
    ...
    private static final String ACTION_SCREEN_OFF_DELAYED = "com.qidizi.netSniffer.SCREEN_OFF_DELAYED";
    ...
    */
            ...
            try {
                ...
                if (cmd == Command.start || cmd == Command.reload) {
                    if (VpnService.prepare(KcaVpnService.this) == null) {
                        ...
                        //
                        prefs.edit().putBoolean(KcaConstants.PREF_VPN_ENABLED, false).apply();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        boolean enabled = prefs.getBoolean(KcaConstants.PREF_VPN_ENABLED, false);

        if (intent != null && intent.hasExtra(EXTRA_REASON)) {
            String reason = intent.getStringExtra(EXTRA_REASON);
            if (reason.equals(KcaConstants.VPN_STOP_REASON)) stopSelf();
        }
    ...
    public static void reload(String reason, Context context) {
        broadcastWidget(context);
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        if (prefs.getBoolean(KcaConstants.PREF_VPN_ENABLED, false)) {
            Intent intent = new Intent(context, KcaVpnService.class);
    ...
    public static void broadcastWidget(Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        boolean is_enabled = prefs.getBoolean(KcaConstants.PREF_VPN_ENABLED, false);
        Intent toggleIntent = new Intent(context, KcaSnifferWidget.class);
        toggleIntent.setAction(KcaConstants.WIDGET_SET_ACTION);
